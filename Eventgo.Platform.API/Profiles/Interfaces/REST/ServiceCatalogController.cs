...
using Eventgo.Platform.API.Profiles.Interfaces.REST.Transform;
using System.Linq;
using Microsoft.AspNetCore.Mvc;
using Swashbuckle.AspNetCore.Annotations;

namespace Eventgo.Platform.API.Profiles.Interfaces.REST;
    ...
    IServiceCatalogQueryService serviceCatalogQueryService) : ControllerBase
{
    ...
    [SwaggerOperation("Create Service Catalog", "Create a new service catalog for a profile.", OperationId = "CreateServiceCatalog")]
    ...
    public async Task<IActionResult> CreateServiceCatalog(int profileId, CreateServiceCatalogResource resource)
    {
        ...
        return CreatedAtAction(nameof(GetServiceCatalogById), new { profileId, serviceCatalogId = serviceCatalog.Id }, resourceResponse);
    }
    ...
    [SwaggerOperation("Get Service Catalog by Id", "Get a service catalog by its identifier.", OperationId = "GetServiceCatalogById")]
    ...
    [SwaggerOperation("Update Service Catalog", "Update an existing service catalog.", OperationId = "UpdateServiceCatalog")]
    [SwaggerResponse(200, "The service catalog was updated.", typeof(ServiceCatalogResource))]
    [SwaggerResponse(404, "The service was not found.")]
    public async Task<IActionResult> UpdateServiceCatalog(int profileId, int serviceCatalogId, UpdateServiceCatalogResource resource)
    {
        var command = UpdateServiceCatalogCommandFromResourceAssembler.ToCommandFromResource(profileId, serviceCatalogId, resource);
        var serviceCatalog = await serviceCatalogCommandService.Handle(command);
        if (serviceCatalog is null) return NotFound();
