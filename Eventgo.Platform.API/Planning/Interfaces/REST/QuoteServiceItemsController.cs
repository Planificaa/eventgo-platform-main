...
[Tags("Quotes")]
public class QuoteServiceItemsController(IServiceItemCommandService serviceItemCommandService, IQuoteQueryService quoteQueryService, IServiceItemQueryService serviceItemQueryService):ControllerBase
{
    ...
    public async Task<IActionResult> GetServiceItemById([FromRoute] string quoteId, [FromRoute] string serviceItemId)
    {
        ...
        var serviceItem = await serviceItemQueryService.Handle(new GetServiceItemByIdQuery(new ServiceItemId(Guid.Parse(serviceItemId))));
    ...
    public async Task<IActionResult> CreateServiceItem([FromRoute]string quoteId,[FromBody] CreateServiceItemResource resource)
    {
        var quote = await quoteQueryService.Handle(new GetQuoteByIdQuery(new QuoteId(Guid.Parse(quoteId))));
        ...
        return CreatedAtAction(nameof(GetServiceItemById), new { quoteId = quote.Id.Identifier.ToString(),serviceItemId = createdResource.Id }, createdResource);
    }
    ...
    public async Task<IActionResult> GetServiceItemsByQuoteId([FromRoute] string quoteId)
    {
        ...
        var serviceItemResources = serviceItems.Select(ServiceItemResourceFromEntityAssembler.ToResourceFromEntity).ToList();
